...
const Favorites = () => {
  ...
  const [data, setData] = useState<DataResponse>({ favoriteList: [] });
  const [error, setError] = useState<string | null>(null);
  const [pageNo, setPageNo] = useState<number>(1);
  const [renderList, setRenderList] = useState<Business[]>([]);
  ...
  const handleFavoriteClick = async (businessId: number) => {
    // Prepare FormData
    const formData = new FormData();
  ...
  const handlePageNumber = (newPageNo: number) => {
