...
const InterestsList = ({
  ...
  const removeTab = useCallback((closeItem: any) => {
    setSelectionList((prevList: any) =>
      prevList.map((category: any) => ({
        ...category,
  ...
  const handleSubmit = async (e) => {
    ...
    const convertedArray = filteredSelectionList.map((item: any) => ({
      ...
      <div className={`interests-filters ${fullwidth ? "w-100" : ""}`}>
        <ul>
          ...
          {selectionList.length > 0 ? (
            selectionList.map((category: any) =>
              category?.subCategoryArray?.length > 0
                ? category.subCategoryArray.map((subCat: any) => (
                    <SubCategoryTab
